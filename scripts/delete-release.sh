...
"
  ...
  echo "The following charts have changed, and their releases will be deleted"
  for chart in "${changed_charts[@]}"; do
    delete_chart_release "$repository" "$chart"
    echo "Deleted release for $chart"
  done
...
function check_gh_login() {
  if ! gh auth status >/dev/null 2>&1; then
    echo "Please login to GitHub using 'gh auth login'." 1>&2
    exit 1
  fi
}
  ...
  while read -r chart; do
    ...
    if [[ -f "$file" ]]; then
      ...
      if [[ "$chart_version" == "$SUPPORT_DELETE_RELEASE_VERSION" ]]; then
        ...
        echo "Chart version $chart_version is not supported for deletion." 1>&2
        exit 1
      fi
    else
...
function lookup_changed_charts() {
  ...
  changed_charts=$(cut -d "/" -f "1-$depth" <<<"$changed_files" | uniq | sort -u | filter_charts)
  # Get changed charts name
  cut -d "/" -f "$depth" <<<"$changed_files"
}

# delete_chart_release deletes a release for a Helm chart.
# Arguments:
#   $1: The repository name.
#   $2: The release name prefix.
#   $3: The chart version.
# Returns:
#   None.
function delete_chart_release() {
  ...
  local chart_version=$(yq eval '.version' "$chart_name/Chart.yaml")
  ...
  if [[ -n "$release_id" ]]; then
    ...
    echo "No release found for $release_name"
  fi
}
